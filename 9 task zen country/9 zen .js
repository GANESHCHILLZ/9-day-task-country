let getCountries = () => {
  ...
  data.onload = () => {
    ...
    let getpopulation = asia.filter(
      (papulations) => papulations.population < 200000
    );
    let papulation = getpopulation.map(
      (populationless) => populationless.name.common
    );
    console.log(papulation);

    //3. Print the total population of countries using reduce function

    let totalpopulation = asia.filter(
      (papulationss) => papulationss.population
    );
